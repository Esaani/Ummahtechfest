...
export default function Ghana2026() {
  const { get } = useCmsSections('ghana_2026')
  const hero = { ...DEFAULT_GHANA_HERO, ...get('ghana-2026-hero') }

  return (
    <main>
      <section className="relative min-h-[70vh] md:min-h-[80vh] flex items-center overflow-hidden pt-24 md:pt-20">
        <div className="absolute inset-0 z-0">
          <img alt="Ummah Tech Fest Ghana" className="w-full h-full object-cover opacity-40" src={hero.image_url} loading="lazy"/>
        ...
        <div className="relative z-10 px-margin-mobile md:px-margin-desktop max-w-container-max mx-auto w-full">
          <div className="max-w-2xl space-y-6 text-center md:text-left" data-aos="fade-right">
            <span className="inline-block px-4 py-1 rounded-full border border-primary-fixed text-primary-fixed label-md uppercase tracking-widest text-[10px] md:text-xs">{hero.badge}</span>
            <h1 className="text-5xl md:headline-xl text-primary leading-none uppercase">{hero.headline}<br/><span className="text-secondary italic">{hero.headline_highlight}</span></h1>
